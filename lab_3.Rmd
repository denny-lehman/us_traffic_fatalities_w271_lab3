...
data_clean <- data %>% mutate(
  ...
  # laws
  speed_limit = factor(
    round(sl55) * 55 + round(sl65) * 65 + round(sl70) * 70 + round(sl75) * 75 + round(slnone) * 100
...
> Linear models are a sensible place to start because they are simple and easy to interpret. Additionally, from the EDA we can see that log_total_fatalities_rate changes linearly over time.
...
mod3 <- lm(total_fatalities_rate ~ unemployment_rate, data=data_clean)
summary(mod3)

example <- loess(total_fatalities_rate ~ unemployment_rate,data= data_clean)
data_clean %>%
...
ggplot(data=data_clean, aes(x=unemployment_rate, y=total_fatalities_rate, col=sl55))+geom_point()+ 
        stat_smooth()

ggplot(data=data_clean, aes(x=unemployment_rate, y=total_fatalities_rate, col=state))+geom_point()+ 
        stat_smooth()

ggplot(data=data_clean, aes(x=unemployment_rate, y=total_fatalities_rate))+geom_point()+ 
        stat_smooth()

ggplot(data=data_clean, aes(x=unemployment_rate, y=total_fatalities_rate, col=bac08))+geom_point()+ 
        stat_smooth(method = 'lm')

# bac is continuous, but has so few distinct values that we can represent it as ordinal values
ggplot(data=data_clean, aes(group=bac10, y=total_fatalities_rate))+geom_boxplot()
ggplot(data=data_clean, aes(group=bac08, y=total_fatalities_rate))+geom_boxplot()

ggplot(data=data_clean, aes(group=factor(seatbelt, levels =c("0","1","2")), y=total_fatalities_rate))+geom_boxplot()

ggplot(data=data, aes(group=factor(year), y=totfatrte))+geom_boxplot()
ggplot(data=data_clean, aes(group=factor(per_se_law), y=total_fatalities_rate))+geom_boxplot()

plot(x=data_clean$unemployment_rate, y=data_clean$total_fatalities_rate, type="p")
...
The random effects model assumes the time-invariant unobserved effect is uncorrelated with the explanatory variables. Other than that, the rest assumptions are the same with the fixed effect model. Whether the time-invariant unobserved effect is uncorrelated with the explanatory variables can be tested with a Hausman test as below. **We reject the null hypothesis, suggesting the random effects model assumption is not met.** ^[We notice that if we use the original total fatalities rate instead of its logged version, the test will fail to reject the null hypothesis. Therefore, the data transformation can also influence the validity of the model choice.] Since the individual specific effects exist and they are correlated with the explanatory variables, the random effect model will suffer from biased coefficient estimates and standard error estimates.
...
In FE model, the coefficient of *log_vehicle_miles_per_capita* is 0.678, which means a unit increase in *log_vehicle_miles_per_capita* will cause the *log_total_fatalities_rate* to increase by 0.678. Therefore, we can calculate the changes to the total fatalities rate based on the changes of *log_vehicle_miles_per_capita*  during boom and bust in the pandemic, as below. However, since each state has different population and fatalities rate, we cannot calculate a specific number of fatalities based on the data above.
