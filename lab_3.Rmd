...
data_clean <- data %>% mutate(
  ...
  year = factor(year),
  state=factor(state),
  speed_limit = factor(round(sl55) * 55 + round(sl65) * 65 + round(sl70) * 70 + round(sl75) * 75 + round(slnone) * 100),# assuming 100 for no speed limit
  seat_belt=factor(if_else(seatbelt == 0, 'none', if_else(seatbelt == 1, 'primary','secondary'))),
  min_age = as.factor(minage),
  # blood_alcohol_limit = factor(round(bac10)*2 + round(bac08) * 1),
  blood_alcohol_limit = factor(if_else(round(bac10) == 1, 'bac10', if_else(round(bac08)==1, 'bac08','none')),labels = c('none','bac08','bac10')),
  ...
  total_fatalities_rate = totfatrte # dependent variable
) %>% dplyr::select(
  year,
  state,
...
expanded_model = plm(
  total_fatalities_rate ~ factor(year) + blood_alcohol_limit + per_se_law +
...
stargazer(
  ...
  fixed_effect_model,
  type = "text",
  keep = c("blood_alcohol_limit", "per_se", "seatbelt"),
